...
def create_directory_tree(startpath, identifier, skip_items):
    ...
    for root, dirs, files in os.walk(startpath):
        # Remove hidden directories and directories in skip_items from dirs list to prevent recursion
...
def main():
    ...
    # =========== ######  SKIP LIST HERE  ###### ===========
    # Add both files and directories to skip
    skip_items = [
        # Files to skip
        "Vm.json", 
        "VmSafe.json",
    ...
    for root, dirs, files in os.walk(start_directory):
        # Skip hidden directories and directories in skip_items
        dirs[:] = [d for d in dirs if not d.startswith('.') and d not in skip_items]
    ...
    # Calculate the total length sum (fixed the unpacking error)
